...
- (id)propertyListForObject:(NSManagedObject *)pObject block:(NSManagedObjectArchivingBlock)block {
	...
	NSDictionary* attributesOriginal = [pObject dictionaryWithValuesForKeys:[[[entityDescription attributesByName] allKeys] mutableCopy]];//entityDescription.attributeKeys];
...
- (id)copyUsingContext:(NSManagedObjectContext *)pContext insert:(BOOL)pInsert {
	...
	[copy setValuesForKeysWithDictionary:[self dictionaryWithValuesForKeys:[[[self.entity attributesByName] allKeys] mutableCopy]]];
	return copy;
}
